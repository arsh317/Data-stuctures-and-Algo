...
& some are scattered. An occupied seat is marked with a character 'x' and an unoccupied seat is marked with 
a dot ('.') Now your target is to make the whole group sit together i.e. next to each other, without having 
any vacant seat between them in such a way that the total number of hops or jumps to move them should be minimum. 
...
int Solution::seats(string A) {
    ...
    int st=Pos[0],en=Pos[n-1];
    
    int mid = n / 2;
    mid = n & 1 ? Pos[mid] : (Pos[mid] + Pos[mid - 1])/2;
    
    long long ans=0; int s=-1;
